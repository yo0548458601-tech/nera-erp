      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-6xl flex-col justify-center">
        <div className="grid gap-6 rounded-[36px] border border-white/10 bg-white/10 p-6 shadow-2xl backdrop-blur lg:grid-cols-[1.05fr_0.95fr] lg:p-10">
          <div className="flex flex-col justify-between rounded-[28px] border border-cyan-400/20 bg-slate-950/70 p-6">
            <div>
              <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-cyan-500 text-2xl font-semibold text-white">N</div>
              <h1 className="mt-6 text-3xl font-semibold">כניסה למערכת</h1>
              <p className="mt-3 text-sm leading-7 text-slate-300">ברוכים הבאים ל-Nera. התחברו לממשק הפלטפורמה, בחרו ארגון והתחילו את העבודה.</p>
            </div>
            <div className="mt-8 rounded-3xl border border-cyan-400/20 bg-cyan-500/10 p-4 text-sm text-cyan-100">
              ...
              <p className="mt-2 leading-7">ההתחברות מיועדת לממשק מאובטח עם סשן שרת-סייד וארכיטקטורת הרשאות עתידית.</p>
            </div>
          </div>

          <form className="rounded-[28px] border border-white/10 bg-slate-900/70 p-6" onSubmit={handleSubmit}>
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-2xl font-semibold">התחברות</h2>
              {demoModeEnabled ? <span className="rounded-full bg-emerald-500/20 px-3 py-1 text-sm text-emerald-200">מצב הדגמה</span> : null}
            </div>
            ...
            <div className="mt-2 flex items-center gap-2 rounded-2xl border border-white/10 bg-slate-800 px-4 py-3">
              ...
              />
              <button type="button" className="text-sm text-cyan-300" onClick={() => setShowPassword((value) => !value)}>
            ...
            <button type="submit" className="mt-6 w-full rounded-2xl bg-cyan-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-70" disabled={isSubmitting}>
              {isSubmitting ? 'מתחבר...' : 'התחברות'}
            </button>

            {demoModeEnabled && onDemoLogin ? (
              <button type="button" onClick={onDemoLogin} className="mt-3 w-full rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-semibold text-emerald-200">
                כניסה למצב הדגמה
              </button>
            ) : null}
