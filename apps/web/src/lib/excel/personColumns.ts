 ...
 */
export function buildPersonExcelColumns(
  customFieldDefinitions: CustomFieldDefinition[],
  customFieldValues: CustomFieldValue[]
): ExcelColumnContract<PersonEntity>[] {
  ...
  const customFieldColumns: ExcelColumnContract<PersonEntity>[] = customFieldDefinitions
    ...
    .map(definition => ({
      ...
      resolveValue: (person: PersonEntity) =>
        customFieldValues.find(
          value => value.entityId === person.id && value.customFieldDefinitionId === definition.id
        )?.value,
