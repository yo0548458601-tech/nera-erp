...
import { demoNotes, demoPersonEntities, demoRoleAssignments } from '../lib/entities/demoEntitiesData';
 ...
 */
function stampContactMethods<TDraft extends { id: string }, TFull extends TDraft & { entityId: string; createdAt: string; updatedAt: string }>(
  drafts: TDraft[],
  entityId: string,
  previous: TFull[],
  now: string,
...
type EntityContextValue = {
  ...
  updatePerson: (entityId: string, input: UpdatePersonInput, updatedByUserId: string) => PersonEntity | undefined;
  addRoleToEntity: (entityId: string, role: EntityRoleId, roleDefinition: RoleDefinition | undefined, organizationId?: string) => boolean;
...
export function EntityProvider({ children }: { children: ReactNode }) {
  ...
  const getEntityById = useCallback((id: string) => entities.find((entity) => entity.id === id), [entities]);
   ...
   */
  const updatePerson = useCallback((entityId: string, input: UpdatePersonInput, updatedByUserId: string): PersonEntity | undefined => {
    ...
    setEntities((current) =>
      current.map((entity) => {
        ...
        const phones = stampContactMethods<PhoneDraft, Phone>(input.phones, entityId, entity.profile.phones, now);
        const emails = stampContactMethods<EmailDraft, Email>(input.emails, entityId, entity.profile.emails, now);
        const addresses = stampContactMethods<AddressDraft, Address>(input.addresses, entityId, entity.profile.addresses, now);
        ...
        const before: Record<string, unknown> = {
          ...
          phones: entity.profile.phones.map((phone) => ({ number: phone.number, type: phone.type, isPrimary: phone.isPrimary, status: phone.status })),
          emails: entity.profile.emails.map((email) => ({ address: email.address, type: email.type, isPrimary: email.isPrimary, status: email.status })),
          addresses: entity.profile.addresses.map((address) => ({ city: address.city, street: address.street, isPrimary: address.isPrimary, status: address.status })),
        };
        const after: Record<string, unknown> = {
          ...
          phones: phones.map((phone) => ({ number: phone.number, type: phone.type, isPrimary: phone.isPrimary, status: phone.status })),
          emails: emails.map((email) => ({ address: email.address, type: email.type, isPrimary: email.isPrimary, status: email.status })),
          addresses: addresses.map((address) => ({ city: address.city, street: address.street, isPrimary: address.isPrimary, status: address.status })),
        };
        const changes = diffEntityFields(entityId, before, after, updatedByUserId, () => createId('change'));
        if (changes.length > 0) {
  ...
  const addRoleToEntity = useCallback((entityId: string, role: EntityRoleId, roleDefinition: RoleDefinition | undefined, organizationId?: string) => {
    let didAdd = false;
    setRoleAssignments((current) => {
      ...
      return [
        ...current,
        { id: createId('role'), entityId, role, organizationId, status: 'active', createdAt: new Date().toISOString() },
      ];
    });
    if (didAdd) {
      setEntities((current) => current.map((entity) => (entity.id === entityId ? { ...entity, updatedAt: new Date().toISOString() } : entity)));
    }
    return didAdd;
  }, []);
  ...
  const getNotesForEntity = useCallback((entityId: string) => notes.filter((note) => note.entityId === entityId), [notes]);

  /** Recomputes an entity's note-count summary; only a genuinely new note advances lastNoteAt. */
  const touchEntityNotesMetadata = useCallback((entityId: string, count: number, newestNoteAt?: string) => {
    setEntities((current) =>
      current.map((entity) =>
        entity.id === entityId
          ? {
              ...entity,
              notesMetadata: { count, lastNoteAt: newestNoteAt ?? entity.notesMetadata.lastNoteAt },
  ...
  const addNote = useCallback(
    (entityId: string, content: string, authorUserId: string) => {
      ...
      setNotes((current) => {
        ...
        const activeCount = next.filter((note) => note.entityId === entityId && !note.deletedAt).length;
    ...
    setNotes((current) =>
      current.map((note) => {
        ...
        return {
          ...
          revisions: [...note.revisions, { id: createId('rev'), content: note.content, editedAt: note.updatedAt, editedByUserId: note.updatedByUserId ?? note.createdByUserId }],
        };
      }),
    );
  }, []);
  ...
  const softDeleteNote = useCallback(
    (noteId: string, deleterUserId: string) => {
      setNotes((current) => {
        ...
        const now = new Date().toISOString();
        const next = current.map((note) => (note.id === noteId ? { ...note, deletedAt: now, deletedByUserId: deleterUserId } : note));
        const activeCount = next.filter((note) => note.entityId === target.entityId && !note.deletedAt).length;
  ...
  const restoreNote = useCallback(
    (noteId: string) => {
      setNotes((current) => {
        ...
        const next = current.map((note) => (note.id === noteId ? { ...note, deletedAt: null, deletedByUserId: null } : note));
        const activeCount = next.filter((note) => note.entityId === target.entityId && !note.deletedAt).length;
        touchEntityNotesMetadata(target.entityId, activeCount);
        return next;
      });
  ...
  const recordDuplicateOverride = useCallback(
    (entityId: string, matchedEntityIds: string[], reasons: DuplicateMatchReason[], reason: string, decidedByUserId: string) => {
      setDuplicateOverrides((current) => [
        ...current,
        { id: createId('override'), entityId, matchedEntityIds, reasons, reason, decidedByUserId, decidedAt: new Date().toISOString() },
      ]);
    },
    [],
  );
