...
/**
 * Unchanged from the pre-P013A client component, just relocated out of
 * `page.tsx` (now a Server Component - see `page.tsx`'s own doc comment).
 * `useEntities()` resolves to the nested, server-seeded `EntityProvider`
 * `page.tsx` mounts around this component.
 */
export function PersonDetailPageClient({ entityId }: { entityId: string }) {
  const { getEntityById } = useEntities();
  const canEditEntity = useMyPermission('entities.edit');
  const person = getEntityById(entityId);
