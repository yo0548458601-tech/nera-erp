...
/**
 * The interactive Contacts list - unchanged from the pre-P013A client
 * component, just relocated out of `page.tsx` (now a Server Component - see
 * `page.tsx`'s own doc comment). Still reads `entities`/`roleAssignments`
 * via `useEntities()`; it now resolves to the *nested*, server-seeded
 * `EntityProvider` `page.tsx` mounts around this component, not the
 * app-shell-level one, so no consumer-side change was needed here.
 */
