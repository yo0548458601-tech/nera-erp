...
- **Consumers:** none yet by way of a real business module — Invoices/Purchase Orders/Payments (its real future consumers) don't exist yet. Unlike the P010–P012 precedent, P014 does ship one real `apps/web` UI page (`app/(app)/operations/documents`, a `DocumentsVerificationPanel` client component) — an explicit verification surface, not a business feature, built so every mandatory flow (upload/reject/persist/signed-URL/soft-delete/restore/hard-delete) could be exercised through the same real request path a future module's UI will use, and so the generated-PDF/font work could be visually confirmed rather than only asserted by automated text extraction. It now also includes an in-page preview action (`DocumentPreviewModal`, PDF/JPEG/PNG only, via `mode: 'view'`) alongside the existing download action.
- **Filename fidelity:** the original (including Hebrew) filename supplied at upload round-trips byte-for-byte through every later view/download — a real Next.js Server Action `FormData` defect (non-ASCII multipart filenames silently decoded as Latin-1 before the Server Action receives them) meant `File.name` could not be trusted; `apps/web/src/lib/actions/originalFilenameMetadata.*` instead carries the exact filename through a UTF-8-safe, Base64URL-encoded sidecar form field, decoded and validated server-side with no fallback to `File.name`.
...
- **Current status:** **partial** — real, tested, and **merged to `main`** (PR #4, merge commit `13e3e3a1ce26077c8a94832504acf75ed0200125`, CI green on the exact PR head commit, run `31089192043`); "partial" reflects that it still has zero real `apps/web` business-module callers (Invoices/Purchase Orders don't exist yet), not incomplete implementation. The `20260803080447_add_document_persistence` migration (`documents`/`document_links` tables, RLS `ENABLE`+`FORCE`, per-table isolation policy, the hand-added `document_links_document_target_active_unique` partial unique index enforcing "one active link per document/target") is applied against the real local database and verified: `documentPersistenceRls.test.ts` is green (introspection + live cross-organization isolation, matching the `entityPersistenceRls.test.ts`/`configurationPersistenceRls.test.ts` pattern). The full repository suite as merged is 482 tests across 51 files (480 passing; 2 local-database-state-only failures in one pre-existing, non-P014 file, not reproduced in CI — see `docs/sprints/P014_SPRINT_CLOSING_REPORT.md`'s Final Known Limitations), including the live `s3StorageProvider.test.ts` SeaweedFS suite and six pre-existing P011–P013B live-RLS test files corrected for the same FORCE-RLS/non-superuser-admin architecture this sprint established (see `packages/database/README.md`) — CI's own `Validate` job (lint/typecheck/test/build) is green on the merged commit; reaching CI-green additionally required six narrowly-scoped CI-environment fixes (SeaweedFS data-directory creation, a readiness-probe fix, a Prettier formatting fix, and a three-commit non-superuser/FORCE-RLS CI role-architecture repair), detailed in `docs/sprints/P014_SPRINT_CLOSING_REPORT.md`. End-to-end functional verification (upload of every allowed type, rejection of every disallowed case, persistence, a real signed-URL round trip against SeaweedFS, a real generated Hebrew PDF visually confirmed correct with its page-2 header repeat, soft-delete/restore, admin hard-delete, cross-org denial) passed by calling the exact Server Action functions the verification UI invokes. The `p014-document-engine` branch is retained (not deleted) per explicit Owner instruction.
