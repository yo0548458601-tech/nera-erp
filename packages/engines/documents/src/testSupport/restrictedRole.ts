/**
 * TEST-ONLY. Never imported by any production file in this package - that
 * is the entire point (P012 owner decision: production runtime code must
 * not contain test-only behavior). Duplicated, not imported, from
 * `@nera/organization-engine`'s own `testSupport/restrictedRole.ts` -
 * `NERA_ARCHITECTURAL_INVARIANTS.md` §10.3 requires a local, duplicated
 * helper per package rather than a cross-package test-only dependency edge.
 *
